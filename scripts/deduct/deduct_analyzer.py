...
Each test block in the log is separated by a header line like:
    Test-Type: insert_tail, Test-Index: 0
...
def parse_log(file_path):
    ...
    DataFrames, each containing columns 'Execution-Time' and 'Class'.
    """
    ...
    with open(file_path, 'r') as f:
        for line in f:
            line = line.strip()
            # Detect header line starting with "Test-Type:"
            if line.startswith("Test-Type:"):
                # If previous test block exists, save its data
                if current_test is not None and data_lines:
                    df = pd.read_csv(StringIO("\n".join(data_lines)),
                                     header=None,
                                     names=['Execution-Time', 'Class'])
                    tests.setdefault(current_test['test_type'], []).append(df)
                    data_lines = []
                # Parse header, e.g., "Test-Type: insert_head, Test-Index: 0"
                m = re.match(r"Test-Type:\s*(\S+),\s*Test-Index:\s*(\d+)", line)
                if m:
                    test_type = m.group(1)
...
def plot_combined_cdf_and_histogram(test_type, df_list):
    ...
    and then plot the CDF and histogram of the 'Execution-Time' column.
    ...
    if 'Execution-Time' not in combined_df.columns:
        combined_df.columns = [col.strip() for col in combined_df.columns]
    if 'Execution-Time' not in combined_df.columns:
        raise KeyError("Column 'Execution-Time' not found in the data.")

    # Convert columns to numeric
    combined_df['Execution-Time'] = pd.to_numeric(combined_df['Execution-Time'], errors='coerce')
    combined_df['Class'] = pd.to_numeric(combined_df['Class'], errors='coerce')

    # Filter data to the specified range [X_MIN, X_MAX]
    combined_df = combined_df[(combined_df['Execution-Time'] >= X_MIN) & (combined_df['Execution-Time'] <= X_MAX)]
    ...
    # -----------------------------
    plt.figure()
    plt.hist(df_class0['Execution-Time'], bins=BINS, alpha=0.5, label='Class 0', edgecolor='black')
    plt.hist(df_class1['Execution-Time'], bins=BINS, alpha=0.5, label='Class 1', edgecolor='black')
    ...
    for class_val, df_class in zip([0, 1], [df_class0, df_class1]):
        exec_times = np.sort(df_class['Execution-Time'].values)
        if len(exec_times) == 0:
...
def plot_combined_kde(test_type, df_list):
    """
    Combine multiple DataFrames (from different test iterations) into one,
    and then plot the KDE curves for the 'Execution-Time' distribution.
    ...
    plt.figure()
    sns.kdeplot(data=df_class0, x='Execution-Time', fill=True, alpha=0.3, label='Class 0', clip=(X_MIN, X_MAX))
    sns.kdeplot(data=df_class1, x='Execution-Time', fill=True, alpha=0.3, label='Class 1', clip=(X_MIN, X_MAX))
    plt.xlabel("Execution Time")
    plt.ylabel("Density")
